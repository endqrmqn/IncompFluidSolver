...
For this derivation we will refer to figure \ref{fig:grid_x}.
Let the location of the top and bottom faces of the shaded box be $y_t$ and $y_b$.
Similarly, let the location of the right and left faces be $x_r$ and $x_l$.
Starting from equation \eqref{eq:ns_equation} in the $x$ direction, we have
\begin{equation}
  \begin{aligned}
    &\int _{\mathcal{V}} \partial_t u\, \mathrm{d}\mathcal{V} + \int_{\mathcal{Y}} u^2(x_r) - u^2(x_l)\,\mathrm{d}\mathcal{Y} + \int_{\mathcal{X}} (uv)(y_t) - (uv)(y_b)\,\mathrm{d}\mathcal{X}  =  \\
    &- \int _{\mathcal{Y}} p(x_r) - p(x_l)\, \mathrm{d}\mathcal{Y} + Re^{-1}\left(\int_{\mathcal{Y}} (\partial_x u)(x_r) - (\partial_x u)(x_l)\,\mathrm{d}\mathcal{Y} + \int_{\mathcal{X}} (\partial_y u)(y_t) - (\partial_y u)(y_b)\,\mathrm{d}\mathcal{X}\right).
  \end{aligned}
\end{equation}
...
\begin{equation}
  \begin{aligned}
    ...
    \int_{\mathcal{Y}} u^2(x_r) - u^2(x_l)\,\mathrm{d}\mathcal{Y} &= \frac{\Delta y}{4}\left(\left(u_{i,j+1} + u_{i,j}\right)^2 - \left(u_{i,j-1} + u_{i,j}\right)^2\right) \\
    \int_{\mathcal{X}} (uv)(y_t) - (uv)(y_b)\,\mathrm{d}\mathcal{X} &= \frac{\Delta x}{4}\left(\left(u_{i+1,j} + u_{i,j}\right)\left(v_{i,j} + v_{i,j+1}\right) - \left(u_{i-1,j} + u_{i,j}\right)\left(v_{i-1,j} + v_{i-1,j+1}\right)\right)\\
    \int _{\mathcal{Y}} p(x_r) - p(x_l)\, \mathrm{d}\mathcal{Y} &= \left(p_{i-1,j} - p_{i-1,j-1}\right)\Delta y \\
    Re^{-1}\int_{\mathcal{Y}} (\partial_x u)(x_r) - (\partial_x u)(x_l)\,\mathrm{d}\mathcal{Y} &= \frac{Re^{-1}}{\Delta x}\left(u_{i,j+1} - 2 u_{i,j} + u_{i,j-1}\right)\Delta y \\
    Re^{-1}\int_{\mathcal{X}} (\partial_y u)(y_t) - (\partial_y u)(y_b)\,\mathrm{d}\mathcal{X} &= \frac{Re^{-1}}{\Delta y}\left(u_{i+1,j} - 2 u_{i,j} + u_{i-1,j}\right)\Delta x
  \end{aligned}
\end{equation}
...
We may recognize that the $y$-momentum is functionally symmetric to the $x$-momentum. It therefore follows that Equation \eqref{eq:ns_equation} allows us to obtain

\begin{align}
&\int_\mathcal V \partial_t v\, d\mathcal V
+ \int_\mathcal X \Big( v^2(y_t) - v^2(y_b) \Big)\, d\mathcal X
+ \int_\mathcal Y \Big( (uv)(x_r) - (uv)(x_l) \Big)\, d\mathcal Y
\\= - &\int_\mathcal X \Big( p(y_t) - p(y_b) \Big)\, d\mathcal X
+ Re^{-1}\!\left(
\int_\mathcal X \Big( (\partial_y v)(y_t) - (\partial_y v)(y_b) \Big)\, d\mathcal X
+ \int_\mathcal Y \Big( (\partial_x v)(x_r) - (\partial_x v)(x_l) \Big)\, d\mathcal Y
\right).
\end{align}

Using the same discretization steps, we may integrate and divide by $\Delta x\Delta y$ to obtain,
...
\begin{empheq}[box=\widefbox]{align*}
  ...
  + &\frac{1}{4\Delta x}
    \Big(
      ...
      - (u_{i+1,j-1}+u_{i,j-1})(v_{i,j-1}+v_{i,j})
    \Big) \\
  = -\,&\frac{p_{i,j-1}-p_{i-1,j-1}}{\Delta y}
   \;+\; Re^{-1}\!
    \left(
      \frac{v_{i,j+1}-2v_{i,j}+v_{i,j-1}}{\Delta x^2}
      + \frac{v_{i+1,j}-2v_{i,j}+v_{i-1,j}}{\Delta y^2}
    \right).
\end{empheq}
